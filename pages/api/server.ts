...
export default async function server(
  ...
  try {
    ...
    const signedPayload = await nftCollection.signature.generate({
      ...
      // claimed once guard
      uid: ethers.utils.hexZeroPad(authorAddress, 32),
      price: "0",
    ...
    const verifyResult = await nftCollection?.signature.verify(signedPayload);

    if (!verifyResult) {
      res.status(400).json({ error: "You have already minted the nft once" });
      return;
    }
